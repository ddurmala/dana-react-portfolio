function Resume() {

    const resumeURL = 'https://drive.google.com/file/d/1igS2bvKwcgxOp1c1gOCDetXXvQMBxn7S/view?usp=sharing'

    return (
        <>
            <section className="text-center p-4 my-5">
                <h5>Click the link below to download my full resume:</h5>
                <p className="my-5 resume-link">

                    <div className="mb-3">
                        <svg xmlns="http://www.w3.org/2000/svg" width="30" height="30" fill="currentColor" className="bi bi-download" viewBox="0 0 16 16">
                            <path d="M.5 9.9a.5.5 0 0 1 .5.5v2.5a1 1 0 0 0 1 1h12a1 1 0 0 0 1-1v-2.5a.5.5 0 0 1 1 0v2.5a2 2 0 0 1-2 2H2a2 2 0 0 1-2-2v-2.5a.5.5 0 0 1 .5-.5" />
                            <path d="M7.646 11.854a.5.5 0 0 0 .708 0l3-3a.5.5 0 0 0-.708-.708L8.5 10.293V1.5a.5.5 0 0 0-1 0v8.793L5.354 8.146a.5.5 0 1 0-.708.708z" />
                        </svg>
                    </div>

                    <a href={resumeURL} download="dc-resume-2024.pdf" >Download Resume</a>
                </p>
            </section>
        </>
    )
}

export default Resume